...
def hilbert_order(lat: np.ndarray, lon: np.ndarray, n_bits: int = 16) -> np.ndarray:
  ...
  Updated for **hilbertcurve ≥ 2.0** which replaces
  `distance_from_coordinates()` with `distance_from_point()` /
  ...
  lat_scaled = ((lat - lat.min()) / (lat.max() - lat.min()) * (2 ** n_bits - 1)).astype(int)
  lon_scaled = ((lon - lon.min()) / (lon.max() - lon.min()) * (2 ** n_bits - 1)).astype(int)

  coords_int = np.stack((lat_scaled, lon_scaled), axis=1).tolist()  # list[list[int]] for API ≥2.0

  hc = HilbertCurve(n_bits, 2)
  # Vectorised call introduced in v2.0
