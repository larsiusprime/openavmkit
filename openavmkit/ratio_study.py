...
class RatioStudyBootstrapped(RatioStudy):
		...
		def __init__(
			...
			med, low, high = stats.calc_cod_bootstrap(predictions / ground_truth, confidence_interval, iterations)
			self.cod = med
			self.cod_ci_low = low
			self.cod_ci_high = high

			med, low, high = stats.calc_cod_bootstrap(stats.trim_outliers(predictions / ground_truth), confidence_interval, iterations)
			self.cod_trim = med
			self.cod_trim_ci_low = low
			self.cod_trim_ci_high = high

			#med, low, high = stats.calc_prd_bootstrap(predictions, ground_truth, confidence_interval, iterations)
			med, low, high = 0, 0, 0
			self.prd = med
			self.prd_ci_low = low
			self.prd_ci_high = high

			#med, low, high = stats.calc_prb(predictions, ground_truth, confidence_interval)
			med, low, high = 0, 0, 0
			self.prb = med
			self.prb_ci_low = low
...
def _add_ratio_study(predictions, ground_truth, cluster, value, catch_all, confidence_interval, iterations, min_sales):
...
def _run_ratio_study_breakdowns(settings: dict, df_sales: pd.DataFrame, confidence_interval=0.95, iterations=10000) -> dict:
		...
		for is_assessor in [True, False]:
			if is_assessor:
				...
				if prediction_field not in df_sales:
					warnings.warn(f"prediction_field '{prediction_field}' not found in df_sales, skipping assessor values")
					continue
			else:
				modeler = "openavmkit"
			...
			results["overall"] = RatioStudyBootstrapped(predictions, ground_truth, confidence_interval, iterations)
				...
				for breakdown in breakdowns:
					...
					if by in cat_fields:
						...
						if quantiles > 0:
							...
							df_sub["quantile"] = pd.cut(df_sub[by], bins=bins, labels=labels, include_lowest=True, duplicates="drop")
							...
							for q in labels:
								...
								cluster, catch_all = _add_ratio_study(predictions, ground_truth, cluster, q_clean, catch_all, confidence_interval, iterations, min_sales)
							...
							value_labels = {}
							for value in values:
								value_labels[value] = f"{value:,.0f} - {value + (slice_size-1):,.0f}"
					...
					if catch_all["count"] > 0:
						rs = RatioStudyBootstrapped(np.array(catch_all["predictions"]), np.array(catch_all["ground_truth"]), confidence_interval, iterations)
						catch_all_count = catch_all["count"]
						other_group = "other group" if catch_all_count <= 1 else "other groups"
						cluster[f"{catch_all_count} {other_group} with count < {min_sales}"] = rs
...
def _write_ratio_study_report(all_results: dict, settings: dict, model_group: str, path: str):
		...
		for modeler in all_results:
			...
			data_untrim.append(f"{overall_entry.cod_ci_low:6.1f} - {overall_entry.cod_ci_high:6.1f}")
			...
			data_trim.append(f"{overall_entry.cod_trim_ci_low:6.1f} - {overall_entry.cod_trim_ci_high:6.1f}")
			...
			for vacant_status in modeler_entry:
				if vacant_status == "overall":
					continue
				vacant_name = "vacant land" if vacant_status == "vacant" else "improved property"
				...
				for breakdown_entry in vacant_status_entry:
					...
					if vacant_status == "vacant" and by in impr_fields:
						continue
					cluster : dict[str, RatioStudyBootstrapped] = breakdown_entry.get("cluster")
