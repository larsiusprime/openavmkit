...
- **Saved model parameters are a separate cache layer with different semantics.** Tunable models (XGBoost / LightGBM / CatBoost / NGBoost via Optuna; GWR / kernel regression via bandwidth search) save their tuned hyperparameters under `<locality>/out/models/<model_group>/.../` as `<slug>_params.json`, `<model_name>_bw.json`, or `kernel_bw.pkl`. **Deleting these forces a fresh hyperparameter search** on the next run. **Keeping them skips the search.** As of 2026-06-18 the Optuna `<slug>_params.json` is **fingerprint-guarded** (`_get_params` in [openavmkit/modeling.py](openavmkit/modeling.py) embeds a `__fingerprint` from feature columns + row count + `n_trials` + seed + `_SEARCH_SPACE_VERSION`): if any of those change — different `ind_vars`, sales window, trial budget, or a bumped tuner search space — the saved params are detected as stale and the model **re-tunes automatically** (no manual delete needed); a loud "saved params … are stale … re-tuning" line prints. **A change the fingerprint can't see** (e.g. you edit raw data values without changing columns/row count) still needs a manual delete. Note `<model_name>_bw.json` / `kernel_bw.pkl` (GWR/kernel) are **not** fingerprint-guarded — delete those manually if their context shifts. **Bump `_SEARCH_SPACE_VERSION` in [openavmkit/tuning.py](openavmkit/tuning.py) whenever you change any tuner's search space** so existing params/journals invalidate. See [advanced_settings.md § 8.4](docs/docs/advanced_settings.md#84-saved-model-parameters--different-semantics).
  ...
  - **`lcomp` (LayeredComp) caches its whole fitted ensemble as portable JSON (when `save_params=True`).** lcomp does no hyperparameter search, but fitting (tree build + per-tree `weight_falloff` search) is expensive and fully determined by `(X_train, hyperparameters, random_state)`. `run_layeredcomp` ([openavmkit/modeling.py](openavmkit/modeling.py)) serializes the fitted ensemble via `layeredcompmodel`'s `to_dict`/`to_json` to `<model_group>/.../lcomp/lcomp_model.json`; on the next run with `use_saved_params=True` it deserializes (`LayeredCompBaggingModel.from_dict`) and predicts, **skipping the entire fit** (both tree build and the falloff search — so re-prediction on unchanged training data is ~instant). Same staleness rules as the params files: a fingerprint (feature columns + row count + hyperparams + seed) and the `layeredcompmodel` version recorded in the cache file guard it, and ANY mismatch (version / fingerprint / read error) falls back to a full fit (never a wrong model). The version check is an exact writer-equals-reader comparison, not a floor: the blob is layeredcompmodel's own format, so any upgrade invalidates the cache and forces one refit. Requires `layeredcompmodel >= 0.3.0` (the `to_dict`/`from_dict` model serialization); JSON not pickle — portable, inspectable, safe to load. Round-trip is verified bit-for-bit by `tests/test_modeling.py::test_lcomp_serialization_roundtrip_matches_full_fit` and end-to-end by `test_lcomp_save_and_reuse_model_cache_roundtrip`. `from_dict` yields a predict-ready model, not a resume-training one.
...
- **Two evaluation paths with different conventions — don't conflate them.** (1) The **benchmark model-comparison holdout** (`_calc_benchmark`, scored against `dep_var_test`, default `sale_price_time_adj` — [openavmkit/model_runner.py](openavmkit/model_runner.py)) ranks models against each other on **time-adjusted** prices. By default (`cv_folds=5`) this holdout is the **full-coverage nested-CV out-of-fold prediction** (every sale predicted by a model that didn't train on it — see `run_one_model_cv`), not a single slice; with `cv_folds<=1` it falls back to the single three-tier split. (2) The **formal ratio study** (`_run_ratio_study_breakdowns` — [openavmkit/ratio_study.py](openavmkit/ratio_study.py) ~line 452) is the IAAO-style `assessor`-vs-`openavmkit` report, and it scores against **raw `sale_price`** restricted to the **lookback window** (`look_back_years`, default 1yr — line 421). Per IAAO *Standard on Ratio Studies* §4.4 (window "ideally no more than one year"; longer only if "adjusted for time as necessary"), raw-price-within-1yr is the standard-endorsed choice, so the formal ratio study needs **no** time-adjustment of its test sales.
- **The benchmark holdout (path 1) is leakage-free w.r.t. training and hyperparameter selection, but mildly optimistic on two *preprocessing* steps fit on all sales.** Under nested CV each fold re-tunes and refits on its own training folds (parcels grouped, post-val excluded), so the out-of-fold prediction is genuinely held out. What remains optimistic: time adjustment (`enrich_time_adjustment`, clean stage — [openavmkit/time_adjustment.py](openavmkit/time_adjustment.py)) and variable auto-reduction (`get_variable_recommendations` — [openavmkit/model_runner.py](openavmkit/model_runner.py)) are fit once across the **full sales set before any split**, so held-out rows still inform the time-adjustment index and the retained feature set. Magnitude is small (low-dimensional, robust). (The formal ratio study, path 2, scores against raw price so the time-adjustment leak doesn't touch its ground truth.)
  ...
  - **The nested-CV OOF is the everyday leakage-correct accuracy signal** (interpolation estimate — random folds are defensible because prices are time-adjusted to the valuation date, removing the dominant temporal leak). **Post-valuation sales remain the pristine forecast check** — they never train under any scenario and are broken out separately in `_calc_benchmark`, which is exactly IAAO App. E.3's Split-Sample Technique (post-date = the honest, un-chaseable holdout). So: leave the everyday all-sales calibration as-is (correct for the roll); the nested OOF gives full-coverage honest accuracy, and the post-val breakout is the forecast/anti-chasing check.
...
- **Two nested CV layers — keep them straight.** (Outer) The **evaluation** CV is the parcel-grouped nested out-of-fold protocol in `run_one_model_cv` ([openavmkit/model_runner.py](openavmkit/model_runner.py)); it produces the full-coverage holdout. (Inner) Within each outer fold (and the single-split path), the **hyperparameter-tuning** CV *selects* hyperparameters on that fold's training data. Both are random/grouped, **not temporal** — fine for selection and interpolation-style evaluation, but not a rolling-origin forecast protocol (the post-val breakout is the forecast check). The inner CV lives in `_xgb_kfold_cv` / `_lightgbm_kfold_cv` / the CatBoost/NGBoost tuners via `_cv_index_splits`: legacy shuffled `KFold` when `cv_inner` is unset, or **parcel-grouped** (`GroupShuffleSplit`/`GroupKFold`) single-split / k-fold under `cv_folds>1` (`cv_inner`, default `auto`). CatBoost's inner CV uses its built-in `cv()` (ungrouped) — a small HP-selection optimism only; the outer holdout stays grouped.
